...
export class PasswordChangeComponent implements OnInit {
	...
	updatePassword() {
		...
		this._userService.changePassword(this.oldPassword, this.newPassword, this.currentUser.email).subscribe(
			() => {
				this.submitting = false;
				this.angulartics2.eventTrack.next({
					action: 'Manual password change: password updated successfully',
				});
				posthog.capture('Manual password change: password updated successfully');
