...
export class ConnectionSettingsComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this._tables.fetchTables(this.connectionID, true).subscribe(
			(res) => {
				if (res.length) {
	...
	createSettings() {
		...
		this._connections.createConnectionSettings(this.connectionID, this.connectionSettings).subscribe(
			() => {
				this.getSettings();
	...
	updateSettings() {
		...
		this._connections.updateConnectionSettings(this.connectionID, this.connectionSettings).subscribe(
			() => {
				this.getSettings();
				this.submitting = false;
				this.angulartics2.eventTrack.next({
	...
	resetSettings() {
		this.submitting = true;
		this._connections.deleteConnectionSettings(this.connectionID).subscribe(
			() => {
				this.getSettings();
				this.submitting = false;
				this.angulartics2.eventTrack.next({
