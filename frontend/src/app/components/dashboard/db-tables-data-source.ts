...
export class TablesDataSource implements DataSource<Object> {
  ...
  formatRow(row, columns) {
    ...
    for (const [columnName, columnStructute] of columns) {
      let type = "";
      if (
        columnStructute.data_type === "tinyint" &&
        columnStructute.character_maximum_length === 1
      ) {
        type = "boolean";
    ...
    if (fetchedTable) {
      fetchedTable
        ...
        .subscribe((res: any) => {
          ...
          const columns = res.structure.reduce((items, item) => {
          ...
          if (res.list_fields.length) {
            orderedColumns = res.structure.sort(
              (fieldA: TableField, fieldB: TableField) =>
                res.list_fields.indexOf(fieldA.column_name) -
                res.list_fields.indexOf(fieldB.column_name)
            );
          } else {
            orderedColumns = [...res.structure];
          }

          if (isTablePageSwitched === undefined)
          ...
          if (
            ...
            this.alert_primaryKeysInfo = {
              ...
              type: AlertType.Info,
              message:
                "We can not provide editing for this table because it does not have primary keys. Please add primary key in your database.",
              actions: [
                {
          ...
          this.foreignKeysList = res.foreignKeys.map((field) => {
            return field["column_name"];
          });
          this.foreignKeys = Object.assign(
            {},
          ...
          if (res.widgets) {
            this.widgetsList = res.widgets.map((widget: Widget) => {
            ...
            /*** for select widget ***/
            const selectWidgets = res.widgets.filter(
              (widget: Widget) => widget.widget_type === "Select"
            );
            this.selectWidgetsOptions = Object.assign(
              {},
              ...selectWidgets.map((widget: Widget) => {
  ...
  getQueryParams(row) {
    const params = Object.fromEntries(
      this.keyAttributes.map((column) => {
        if (this.foreignKeysList.includes(column.column_name)) {
          const referencedColumnNameOfForeignKey =
            this.foreignKeys[column.column_name].referenced_column_name;
          return [
            column.column_name,
            row[column.column_name][referencedColumnNameOfForeignKey],
          ];
        }
