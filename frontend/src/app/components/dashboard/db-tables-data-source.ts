		...
		if (fetchedTable) {
			fetchedTable
				...
				.subscribe((res: any) => {
					...
					this.tableBulkActions = res.action_events.filter(
						(action: CustomEvent) => action.type === CustomActionType.Multiple,
					);
					...
					if (res.table_settings.list_fields.length) {
						orderedColumns = res.structure.sort(
							(fieldA: TableField, fieldB: TableField) =>
								res.table_settings.list_fields.indexOf(fieldA.column_name) -
								res.table_settings.list_fields.indexOf(fieldB.column_name),
						);
					} else {
