...
export class SavedFiltersPanelComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
					...
					this._tables.getSavedFilters(this.connectionID, this.selectedTableName).subscribe({
						next: (data) => {
							if (data) {
								...
								this.savedFilterData = data.sort(
									(a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(),
								);
								this.savedFilterMap = Object.assign(
									{},
									...data.map((filter) => {
		...
		this.tableRowStructure = Object.assign(
			{},
			...this.structure.map((field: TableField) => {
	...
	loadSavedFilters() {
		...
		this._tables.getSavedFilters(this.connectionID, this.selectedTableName).subscribe({
			next: (data) => {
				if (data) {
					this.savedFilterData = data.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
					this.savedFilterMap = Object.assign(
						{},
						...data.map((filter) => {
							const transformedFilter = this.processFiltersData(filter);
	...
	processFiltersData(filter: any) {
		...
		transformedFilter.staticFilters = this.getFilterEntries(filter.filters).filter(
			(entry) => !filter.dynamic_column || entry.column !== filter.dynamic_column.column_name,
		);
	...
	applyDynamicColumnChanges() {
		...
		const filterValue =
			selectedFilter.dynamicColumn.value === '' || selectedFilter.dynamicColumn.value === undefined
				? null
				: selectedFilter.dynamicColumn.value;

		const filters = { ...selectedFilter.filters };

		if (selectedFilter.dynamicColumn.column && selectedFilter.dynamicColumn.operator && filterValue !== null) {
