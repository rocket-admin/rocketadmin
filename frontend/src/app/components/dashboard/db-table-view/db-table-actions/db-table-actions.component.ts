...
export class DbTableActionsComponent implements OnInit {
	...
	async ngOnInit() {
		...
		try {
			...
			this.title.setTitle(
				`${this.rulesData.display_name || this.normalizedTableName} - Actions | ${this._company.companyTabTitle || 'Rocketadmin'}`,
			);
		} catch (error) {
	...
	addRule() {
		...
		this._tables.saveRule(this.connectionID, this.tableName, this.selectedRule).subscribe(
			async (res) => {
				this.submitting = false;
				this.angulartics2.eventTrack.next({
					action: 'Rules: rule is saved successfully',
				});
				posthog.capture('Rules: rule is saved successfully');
				try {
	...
	updateRule() {
		...
		this._tables.updateRule(this.connectionID, this.tableName, this.selectedRule).subscribe(
			async (res) => {
				this.submitting = false;
				try {
					const undatedRulesData = await this.getRules();
