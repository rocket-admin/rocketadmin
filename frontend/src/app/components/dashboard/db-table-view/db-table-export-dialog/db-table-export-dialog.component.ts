...
@Component({
	...
	imports: [CommonModule, FormsModule, MatButtonModule, MatDialogModule, MatInputModule, MatRadioModule],
})
export class DbTableExportDialogComponent {
	...
	exportCSV() {
		...
		this._tables.exportTableCSV(this.data).subscribe(
			(res) => {
				...
				posthog.capture('Dashboard: db export is successful');
			},
			(_err) => {
				this.submitting = false;
			},
