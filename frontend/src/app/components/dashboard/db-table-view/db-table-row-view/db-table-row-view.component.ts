...
import { recordViewFieldTypes, UIwidgets } from 'src/app/consts/record-view-types';
...
export class DbTableRowViewComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		this.selectedRowCast = this._tableState.cast.subscribe((row) => {
			...
			if (row?.columnsOrder) {
				const columnsOrder = this.selectedRow.columnsOrder.length
					? this.selectedRow.columnsOrder
					: Object.keys(this.selectedRow.record);
				...
				if (row.relatedRecords?.referenced_by?.length) {
					...
					this.referencedTables = row.relatedRecords.referenced_by.map((table: any) => {
						return { ...table, displayTableName: table.display_name || normalizeTableName(table.table_name) };
					});

					this.referencedTablesURLParams = row.relatedRecords.referenced_by.map((table: any) => {
						const params = {
							[table.column_name]: {
							...
							.subscribe((res) => {
								...
								if (!res.identity_column && !res.list_fields.length) {
									identityColumn = {
										...
										displayName:
											tableWidgetsNameMap[res.structure[0].column_name] ||
								...
								const tableRecords = {
									...
									links: res.rows.map((row) => {
										return res.primaryColumns.reduce(
											(keys, column) => {
