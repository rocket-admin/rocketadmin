	...
	handleConfirmedActions() {
		...
		if (this.data.id) {
			...
			if (this.data.primaryKeys.length === 1) {
				this._tableRow.deleteTableRow(this.connectionID, this.selectedTableName, this.data.primaryKeys[0]).subscribe(
					() => {
						this.onActionsComplete();
					},
					() => {
				...
				this._tables.bulkDelete(this.connectionID, this.selectedTableName, this.data.primaryKeys).subscribe(
					() => {
						this.onActionsComplete();
					},
					() => {
