...
export class DbTablesListComponent implements OnInit, OnChanges {
	...
	searchSubstring() {
		...
		this.foundTables = allTables.filter(
			(tableItem) =>
		...
		// Remove duplicates
		this.foundTables = this.foundTables.filter(
			(table, index, self) => index === self.findIndex((t) => t.table === table.table),
		);
	...
	getFolderIconColor(folder: Folder, isActive?: boolean): string {
		if (isActive) {
			return '#2563EB'; // Primary blue for active folders
		}

		// Check if we're in dark theme
		if (window.matchMedia?.('(prefers-color-scheme: dark)').matches) {
			return folder.iconColor || '#2563EB';
		}
	...
	private preserveFolderStates() {
		...
		console.log(
			'Expanded folders count:',
			Object.values(this.preservedFolderStates).filter((expanded) => expanded).length,
		);
	}
		...
		this._connectionsService.getTablesFolders(this.connectionID).subscribe({
			next: (categories: TableCategory[]) => {
				if (categories && categories.length > 0) {
					...
					console.log(
						'Folders loaded from connection settings:',
						this.folders.map((c) => ({ name: c.name, expanded: c.expanded })),
					);
				} else {
