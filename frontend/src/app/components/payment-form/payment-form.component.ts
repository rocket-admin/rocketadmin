...
export class PaymentFormComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.subscriptionInfo = [
			{
				plan: this.plan,
				chargeDate: this.endOfTrialDate,
				amount: `${this.price} * n`,
				users: 'per each 10 user',
		...
		this._userService.cast.subscribe((user) => {
			...
			console.log(user);
			if (user?.company.id)
				this._paymentService.createIntentToSubscription(user.company.id).subscribe((res) => {
					...
					if (result.error) {
						...
						if (result.setupIntent.status === 'succeeded') {
							this._paymentService
								.createSubscription(this.companyId, result.setupIntent.payment_method, this.subscriptionLevel)
								.subscribe((_res) => {
