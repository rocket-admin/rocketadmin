...
export class DbConnectionConfirmDialogComponent implements OnInit {
	...
	editConnection() {
		this._connections.updateConnection(this.data.dbCreds, this.data.masterKey).subscribe(
			() => {
	...
	createConnection() {
		this._connections.createConnection(this.data.dbCreds, this.data.masterKey).subscribe(
			(res: any) => {
