...
@Component({
	...
	imports: [MatDialogModule, FormsModule, MatButtonModule, MatFormFieldModule, MatInputModule, MatRadioModule],
})
export class DbConnectionDeleteDialogComponent implements OnInit {
	...
	deleteConnection() {
		...
		this._connectionsService.deleteConnection(this.data.id, metadata).subscribe(
			() => {
				...
				posthog.capture('Connect DB: connection is deleted', {
					id: this.data.id,
					title: this.data.title || null,
					reason: this.reason,
					message: this.message,
