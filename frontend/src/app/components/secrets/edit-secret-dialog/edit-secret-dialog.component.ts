...
export class EditSecretDialogComponent {
	...
	onSubmit(): void {
		...
		this._secrets
			.updateSecret(this.data.secret.slug, payload, this.data.secret.masterEncryption ? this.masterPassword : undefined)
			.subscribe({
				next: () => {
					this.angulartics2.eventTrack.next({
						action: 'Secrets: secret updated successfully',
					});
					posthog.capture('Secrets: secret updated successfully');
