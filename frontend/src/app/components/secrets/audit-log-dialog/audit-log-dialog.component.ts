...
import { AuditLogEntry, Secret, SecretPagination } from 'src/app/models/secret';
...
export class AuditLogDialogComponent implements OnInit {
	...
	constructor(
		...
		_dialogRef: MatDialogRef<AuditLogDialogComponent>,
	...
	loadAuditLog(): void {
		this.loading = true;
		this._secrets
			.getAuditLog(this.data.secret.slug, this.pagination.currentPage, this.pagination.perPage)
			.subscribe((response) => {
				if (response) {
