...
export class LoginComponent implements OnInit, AfterViewInit {
	...
	ngAfterViewInit() {
		if (this.isSaas) {
			...
			gAccounts.id.renderButton(document.getElementById('google_login_button'), {
				theme: 'outline',
				size: 'large',
				width: 360,
				text: 'continue_with',
	...
	requestUserCompanies() {
		...
		this._auth.fetchUserCompanies(this.user.email).subscribe((companies) => {
			this.angulartics2.eventTrack.next({
	...
	loginWith2FA() {
		this.submitting = true;
		this._auth.loginWith2FA(this.authCode).subscribe(
			() => {
				this.submitting = false;
				this.angulartics2.eventTrack.next({
