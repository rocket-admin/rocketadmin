...
export class DbTableRowEditComponent implements OnInit {
		...
		this.routeSub = this.route.queryParams.subscribe((params) => {
			...
			if (Object.keys(params).length === 0) {
				this._tables.fetchTableStructure(this.connectionID, this.tableName).subscribe((res) => {
					...
					this.shownRows = this.getModifyingFields(res.structure).filter(
						(field: TableField) => !res.excluded_fields.includes(field.column_name),
					);
				...
				this._tableRow.fetchTableRow(this.connectionID, this.tableName, params).subscribe(
					(res) => {
						this.dispalyTableName = res.display_name || normalizeTableName(this.tableName);
						...
						this.nonModifyingFields = res.structure
							.filter(
								(field: TableField) =>
									!this.getModifyingFields(res.structure).some(
										(modifyingField) => field.column_name === modifyingField.column_name,
									),
							)
							.map((field: TableField) => field.column_name);
						...
						if (this.connectionType === DBtype.Dynamo || this.connectionType === DBtype.ClickHouse) {
							this.readonlyFields = [
								...this.readonlyFields,
								...res.primaryColumns.map((field: TableField) => field.column_name),
							];
						}
						this.tableForeignKeys = res.foreignKeys;
						...
						if (
							res.referenced_table_names_and_columns &&
							res.referenced_table_names_and_columns.length > 0 &&
							res.referenced_table_names_and_columns[0].referenced_by[0] !== null
						) {
							this.isDesktop = window.innerWidth >= 1280;

							this.referencedTables = res.referenced_table_names_and_columns[0].referenced_by.map((table: any) => {
								return { ...table, displayTableName: table.display_name || normalizeTableName(table.table_name) };
							});

							this.referencedTablesURLParams = res.referenced_table_names_and_columns[0].referenced_by.map(
								(table: any) => {
									const params = {
										[table.column_name]: {
							...
							res.referenced_table_names_and_columns[0].referenced_by.forEach((table: any) => {
								...
								this._tables
									...
									.subscribe((res) => {
										const foreignKeysList = res.foreignKeys.map(
											(foreignKey: TableForeignKey) => foreignKey.column_name,
										);
										this.relatedRecordsProperties = Object.assign({}, this.relatedRecordsProperties, {
											[table.table_name]: {
												...
												foreignKeys: Object.assign(
													{},
													...res.foreignKeys.map((foreignKey: TableForeignKey) => ({
														[foreignKey.column_name]: foreignKey,
													})),
												),
												foreignKeysList,
												widgets: Object.assign(
													{},
													...res.widgets.map((widget: Widget) => {
														let parsedParams;
										...
										if (res.rows?.length) {
											...
											const relatedTableForeignKeys = Object.assign(
												{},
												...res.foreignKeys.map((foreignKey: TableForeignKey) => ({
													[foreignKey.column_name]: foreignKey,
												})),
											...
											this._tableRow
												.fetchTableRow(
													...
													res.primaryColumns.reduce((keys, column) => {
														if (
															res.foreignKeys.map((foreignKey) => foreignKey.column_name).includes(column.column_name)
														) {
															const referencedColumnNameOfForeignKey =
																relatedTableForeignKeys[column.column_name]?.referenced_column_name;
												...
												.subscribe((res) => {
													this.relatedRecordsProperties[table.table_name].relatedRecords =
														res.referenced_table_names_and_columns[0];
												});
										}
										...
										const formattedRows = res.rows.map((row) => {
											...
											for (const key in row) {
												if (foreignKeyMap[key] && typeof row[key] === 'object' && row[key] !== null) {
													...
													formattedRow[key] = formatFieldValue(
														row[key],
														res.structure.find((field: TableField) => field.column_name === key)?.data_type || 'text',
													);
												}
											}
											return formattedRow;
										...
										if (res.identity_column && res.list_fields.length) {
											identityColumn = {
												...
												displayName:
													this.relatedRecordsProperties[table.table_name].widgets[res.identity_column]?.name ||
											...
											fieldsOrder = res.list_fields
												...
												.map((field: string) => {
													return {
														fieldName: field,
														displayName:
															this.relatedRecordsProperties[table.table_name].widgets[field]?.name ||
											...
											fieldsOrder = res.structure
												...
												.map((field: TableField) => {
													return {
														...
														displayName:
															this.relatedRecordsProperties[table.table_name].widgets[field.column_name]?.name ||
															normalizeFieldName(field.column_name),
													};
												});
										}
										...
										if (!res.identity_column && res.list_fields.length) {
											identityColumn = {
												...
												displayName:
													this.relatedRecordsProperties[table.table_name].widgets[res.list_fields[0]]?.name ||
													normalizeFieldName(res.list_fields[0]),
											};
											fieldsOrder = res.list_fields.slice(1, 4).map((field: string) => {
												return {
													fieldName: field,
													displayName:
														this.relatedRecordsProperties[table.table_name].widgets[field]?.name ||
										...
										if (!res.identity_column && !res.list_fields.length) {
											identityColumn = {
												...
												displayName:
													this.relatedRecordsProperties[table.table_name].widgets[res.structure[0].column_name]?.name ||
													normalizeFieldName(res.structure[0].column_name),
											};
											fieldsOrder = res.structure.slice(1, 4).map((field: TableField) => {
												return {
													fieldName: field.column_name,
													displayName:
														this.relatedRecordsProperties[table.table_name].widgets[field.column_name]?.name ||
	...
	setRowStructure(structure: TableField[]) {
		...
		const foreignKeysList = this.tableForeignKeys.map((field: TableForeignKey) => {
			return field.column_name;
		});
	...
	getModifyingFields(fields) {
		return fields.filter(
			(field: TableField) =>
				...
				!(
					timestampTypes.includes(field.data_type) &&
					field.column_default &&
					defaultTimestampValues[this.connectionType].includes(field.column_default.toLowerCase().replace(/\(.*\)/, ''))
				),
		);
	}
	...
	getFormattedUpdatedRow = () => {
		...
		if (this.connectionType === DBtype.MySQL) {
			const datetimeFields = Object.entries(this.tableTypes).filter(
				([_key, value]) => value === 'datetime' || value === 'timestamp',
			);
			if (datetimeFields.length) {
				for (const datetimeField of datetimeFields) {
					if (updatedRow[datetimeField[0]]) {
						updatedRow[datetimeField[0]] = updatedRow[datetimeField[0]]
							.replace('T', ' ')
							.replace('Z', '')
							.split('.')[0];
					}
			...
			const dateFields = Object.entries(this.tableTypes).filter(([_key, value]) => value === 'date');
	...
	addRow(continueEditing: boolean) {
		...
		this._tableRow.addTableRow(this.connectionID, this.tableName, formattedUpdatedRow).subscribe(
			(res) => {
	...
	handleActivateAction(action: CustomEvent) {
		if (action.require_confirmation) {
			...
			this._tables
				.activateActions(this.connectionID, this.tableName, action.id, action.title, [this.keyAttributesFromURL])
				.subscribe((res) => {
					if (res?.location)
