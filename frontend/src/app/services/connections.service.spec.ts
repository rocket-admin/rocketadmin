...
describe('ConnectionsService', () => {
	...
	it('should set connectionInfo if connection exists', () => {
		const fakeFetchConnection = vi.spyOn(service, 'fetchConnection').mockReturnValue(
			of({
	...
	it('should call fetchAuditLog', () => {
		...
		const req = httpMock.expectOne(
			`/logs/12345678?page=2&perPage=10&tableName=users_table&email=eric.cartman@south.park`,
		);
		expect(req.request.method).toBe('GET');
		req.flush({});
	...
	it('should call createConnectionSettings and show success snackbar', () => {
		...
		service
			.createConnectionSettings('12345678', { hidden_tables: ['users', 'orders'], default_showing_table: '' })
			.subscribe((_res) => {
				expect(fakeNotifications.showSuccessSnackbar).toHaveBeenCalledWith(
					'Connection settings has been created successfully.',
				);
				isSubscribeCalled = true;
			});
	...
	it('should call updateConnectionSettings and show success snackbar', () => {
		...
		service
			.updateConnectionSettings('12345678', {
				hidden_tables: ['users', 'orders', 'products'],
				default_showing_table: 'users',
			})
			.subscribe((_res) => {
				expect(fakeNotifications.showSuccessSnackbar).toHaveBeenCalledWith(
					'Connection settings has been updated successfully.',
				);
				isSubscribeCalled = true;
			});
	...
	it('should call deleteConnectionSettings and show success snackbar', () => {
		...
		service.deleteConnectionSettings('12345678').subscribe((_res) => {
			expect(fakeNotifications.showSuccessSnackbar).toHaveBeenCalledWith(
				'Connection settings has been removed successfully.',
			);
			isSubscribeCalled = true;
		});
