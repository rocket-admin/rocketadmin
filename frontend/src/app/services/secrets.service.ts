...
export class SecretsService {
	...
	fetchSecrets(page: number = 1, limit: number = 20, search?: string): Observable<SecretListResponse> {
		...
		return this._http.get<SecretListResponse>('/secrets', { params }).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
	...
	createSecret(payload: CreateSecretPayload): Observable<Secret> {
		return this._http.post<Secret>('/secrets', payload).pipe(
			map((res) => {
	...
	updateSecret(slug: string, payload: UpdateSecretPayload, masterPassword?: string): Observable<Secret> {
		...
		return this._http.put<Secret>(`/secrets/${slug}`, payload, { headers }).pipe(
			map((res) => {
	...
	deleteSecret(slug: string): Observable<DeleteSecretResponse> {
		return this._http.delete<DeleteSecretResponse>(`/secrets/${slug}`).pipe(
			map((res) => {
	...
	getAuditLog(slug: string, page: number = 1, limit: number = 50): Observable<AuditLogResponse> {
		...
		return this._http.get<AuditLogResponse>(`/secrets/${slug}/audit-log`, { params }).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
