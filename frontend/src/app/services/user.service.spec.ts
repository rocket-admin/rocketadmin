...
describe('UserService', () => {
	...
	it('should call requestEmailChange', () => {
		...
		service.requestEmailChange().subscribe((res) => {
			...
			expect(fakeNotifications.showAlert).toHaveBeenCalledWith(
				AlertType.Info,
				'Link has been sent to your email. Please check it.',
				[
	...
	it('should call requestPasswordReset and show Success alert', () => {
		...
		service.requestPasswordReset('john@smith.com', 'company_1').subscribe((res) => {
			expect(res).toEqual(requestResponse);
			expect(fakeNotifications.showAlert).toHaveBeenCalledWith(AlertType.Success, 'Check your email.', [
				expect.objectContaining({
					type: AlertActionType.Button,
					caption: 'Dismiss',
				}),
	...
	it('should fall for confirm2FA and show Error alert', async () => {
