...
export class AuthService {
	...
	loginUser(userData: ExistingAuthUser) {
		...
		return this._http.post<any>('/user/login', userData).pipe(
			map((res) => {
				this.auth.next(res);
				return res;
			}),
	...
	loginWith2FA(code: string) {
		return this._http.post<any>('/user/otp/login', { otpToken: code }).pipe(
			map((res) => {
				this.auth.next(res);
				return res;
			}),
	...
	loginWithGoogle(token: string) {
		...
		return this._http.post<any>(config.saasURL + '/saas/user/google/login', { token }).pipe(
			map((res) => {
				this.auth.next(res);
				return res;
			}),
	...
	requestEmailVerifications() {
		return this._http.get<any>('/user/email/verify/request').pipe(
			map((res) => {
	...
	verifyEmail(token: string) {
		return this._http.get<any>(`/user/email/verify/${token}`).pipe(
			map((res) => {
	...
	fetchUserCompanies(email: string) {
		return this._http.get<any>(`/company/my/email/${email}`).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
				this._notifications.showAlert(
	...
	checkInvitationAvailability(token: string) {
		return this._http.get<any>(`/company/invite/verify/${token}`).pipe(
			map((res) => {
				return res;
			}),
			catchError((err) => {
	...
	acceptCompanyInvitation(token: string, password: string, userName: string) {
		return this._http
			.post<any>(`/company/invite/verify/${token}`, {
				password,
				userName,
			})
			.pipe(
	...
	logOutUser() {
		return this._http.post<any>('/user/logout', undefined).pipe(
			map(() => {
