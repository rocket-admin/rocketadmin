...
export class UserService {
	...
	fetchUser() {
		return this._http.get<any>('/user').pipe(
			map((res) => {
				console.log(res);
				this.user.next(res);
				return res;
	...
	sendUserAction(message: string) {
		return this._http.post<any>('/action', { message: message }).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
				return EMPTY;
	...
	requestEmailChange() {
		return this._http.get<any>(`/user/email/change/request`).pipe(
			map((res) => {
	...
	changeEmail(token: string, newEmail: string) {
		return this._http.post<any>(`/user/email/change/verify/${token}`, { email: newEmail }).pipe(
			map((res) => {
	...
	requestPasswordReset(email: string, companyId: string) {
		return this._http.post<any>(`/user/password/reset/request`, { email, companyId }).pipe(
			map((res) => {
	...
	resetPassword(token: string, newPassword: string) {
		return this._http.post<any>(`/user/password/reset/verify/${token}`, { password: newPassword }).pipe(
			map((res) => {
	...
	changePassword(oldPassword: string, newPassword: string, email: string) {
		return this._http.post<any>(`/user/password/change/`, { email, oldPassword, newPassword }).pipe(
			map((res) => {
	...
	changeUserName(userName: string) {
		return this._http.put<any>(`/user/name`, { name: userName }).pipe(
			map((res) => {
	...
	switchOn2FA() {
		return this._http.post<any>('/user/otp/generate', {}).pipe(
			map((res) => {
				return res;
			}),
			catchError((err) => {
	...
	confirm2FA(code: string) {
		return this._http.post<any>('/user/otp/verify', { otpToken: code }).pipe(
			map((res) => {
	...
	switchOff2FA(code: string) {
		return this._http.post<any>('/user/otp/disable', { otpToken: code }).pipe(
			map((res) => {
	...
	getAPIkeys() {
		return this._http.get<any>(`/apikeys`).pipe(
			map((res) => {
				return res;
			}),
			catchError((err) => {
	...
	generateAPIkey(title: string) {
		return this._http.post<any>('/apikey', { title }).pipe(
			map((res) => {
	...
	deleteAPIkey(apiKey: ApiKey) {
		return this._http.delete<any>(`/apikey/${apiKey.id}`).pipe(
			map((res) => {
	...
	deleteAccount(metadata) {
		return this._http.put<any>(`/user/delete`, metadata).pipe(
			map(() => {
