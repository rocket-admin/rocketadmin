...
export class CompanyService {
	...
	fetchCompany() {
		return this._http.get<any>(`/company/my/full`).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
	...
	fetchCompanyMembers(companyId: string) {
		return this._http.get<any>(`/company/users/${companyId}`).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
				this._notifications.showAlert(
	...
	fetchCompanyName(companyId: string) {
		return this._http.get<any>(`/company/name/${companyId}`).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
	...
	updateCompanyName(companyId: string, name: string) {
		return this._http.put<any>(`/company/name/${companyId}`, { name }).pipe(
	...
	revokeInvitation(companyId: string, email: string) {
		return this._http.put<any>(`/company/invitation/revoke/${companyId}`, { email }).pipe(
			map(() => {
	...
	removeCompanyMemder(companyId: string, userId: string, email: string, userName: string) {
		return this._http.delete<any>(`/company/${companyId}/user/${userId}`).pipe(
			map((res) => {
	...
	updateCompanyMemberRole(companyId: string, userId: string, role: CompanyMemberRole) {
		return this._http.put<any>(`/company/users/roles/${companyId}`, { users: [{ userId, role }] }).pipe(
			map((res) => {
	...
	suspendCompanyMember(companyId: string, usersEmails: string[]) {
		return this._http.put<any>(`/company/users/suspend/${companyId}`, { usersEmails }).pipe(
			map((res) => {
	...
	restoreCompanyMember(companyId: string, usersEmails: string[]) {
		return this._http.put<any>(`/company/users/unsuspend/${companyId}`, { usersEmails }).pipe(
			map((res) => {
	...
	getCustomDomain(companyId: string) {
		...
		return this._http.get<any>(config.saasURL + `/saas/custom-domain/${companyId}`).pipe(
			map((res) => res),
			catchError((err) => {
				console.log(err);
				this._notifications.showErrorSnackbar(err.error?.message || err.message);
	...
	createCustomDomain(companyId: string, hostname: string) {
		...
		return this._http.post<any>(config.saasURL + `/saas/custom-domain/register/${companyId}`, { hostname }).pipe(
			map((res) => {
	...
	updateCustomDomain(companyId: string, hostname: string) {
		...
		return this._http.put<any>(config.saasURL + `/saas/custom-domain/update/${companyId}`, { hostname }).pipe(
			map((res) => {
	...
	deleteCustomDomain(companyId: string) {
		...
		return this._http.delete<any>(config.saasURL + `/saas/custom-domain/delete/${companyId}`).pipe(
			map((res) => {
