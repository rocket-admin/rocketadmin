...
export class DataAccessObjectOracle extends BasicDataAccessObject implements IDataAccessObject {
	...
	public async getRowsFromTable(
		...
		if (autocompleteFields?.value && autocompleteFields?.fields?.length > 0) {
			...
			const validFieldNames = tableStructure.map((col) => col.column_name);
			const validatedAutocompleteFields = autocompleteFields.fields.filter((field) => validFieldNames.includes(field));
			if (validatedAutocompleteFields.length === 0) {
				throw new Error('Invalid autocomplete fields provided');
			}

			const andWhere = validatedAutocompleteFields
				.map((_, i) => `${i === 0 ? ' WHERE' : ' OR'} CAST (?? AS VARCHAR (255)) LIKE ?`)
				.join('');

			const likeParams = validatedAutocompleteFields.map(() => `${autocompleteFields.value}%`);
			...
			const rows = await knex.transaction(async (trx) => {
				try {
					const result = await knex
						.raw(
							`SELECT ${validatedAutocompleteFields.map((_) => '??').join(', ')}
         FROM ${tableName} ${andWhere} FETCH FIRST ${DAO_CONSTANTS.AUTOCOMPLETE_ROW_LIMIT} ROWS ONLY`,
							[...validatedAutocompleteFields, ...validatedAutocompleteFields, ...likeParams],
						)
						.transacting(trx);
					trx.commit();
					return result;
