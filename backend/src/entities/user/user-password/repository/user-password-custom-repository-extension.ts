...
export const userPasswordResetCustomRepositoryExtension = {
	...
	async createOrUpdatePasswordResetEntity(
		user: UserEntity,
	): Promise<{ entity: PasswordResetEntity; rawToken: string }> {
		const qb = this.createQueryBuilder('password_reset')
			.leftJoinAndSelect('password_reset.user', 'user')
		...
		newResetPasswordRequest.verification_string = Encryptor.hashVerificationToken(rawToken);
		newResetPasswordRequest.user = user;
		const entity = await this.save(newResetPasswordRequest);
