	...
	protected async implementation(inputData: CreateTableWidgetsDs): Promise<Array<FoundTableWidgetsDs>> {
		...
		for (const widget of widgets) {
			...
			if (availableWidgetIndex >= 0) {
				...
				const createdTableWidget =
					await this._dbContext.tableWidgetsRepository.saveNewOrUpdatedTableWidget(newTableWidget);
		...
		const savedSettings: TableSettingsEntity =
