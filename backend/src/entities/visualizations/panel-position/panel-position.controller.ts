...
export class DashboardWidgetController {
	...
	@ApiOperation({ summary: 'Create a new panel position in a dashboard' })
	@ApiResponse({
		status: 201,
		description: 'Panel position created.',
		type: FoundPanelPositionDto,
	})
	@ApiBody({ type: CreatePanelPositionDto })
	@ApiParam({ name: 'dashboardId', required: true })
	@ApiParam({ name: 'connectionId', required: true })
	@UseGuards(ConnectionEditGuard)
	@Post('/dashboard/:dashboardId/panel-position/:connectionId')
	async createPanelPosition(
		@SlugUuid('connectionId') connectionId: string,
		@Param('dashboardId') dashboardId: string,
		@MasterPassword() masterPwd: string,
		@UserId() userId: string,
	...
	@ApiOperation({ summary: 'Update a panel position in a dashboard' })
	@ApiResponse({
		status: 200,
		description: 'Panel position updated.',
		type: FoundPanelPositionDto,
	})
	@ApiBody({ type: UpdatePanelPositionDto })
	@ApiParam({ name: 'dashboardId', required: true })
	@ApiParam({ name: 'panelPositionId', required: true })
	@ApiParam({ name: 'connectionId', required: true })
	@UseGuards(ConnectionEditGuard)
	@Put('/dashboard/:dashboardId/panel-position/:panelPositionId/:connectionId')
	async updatePanelPosition(
		@SlugUuid('connectionId') connectionId: string,
		@Param('dashboardId') dashboardId: string,
		@Param('panelPositionId') panelPositionId: string,
		@MasterPassword() masterPwd: string,
		@UserId() userId: string,
		@Body() updateDto: UpdatePanelPositionDto,
	): Promise<FoundPanelPositionDto> {
		const inputData: UpdatePanelPositionDs = {
			widgetId: panelPositionId,
			dashboardId,
			connectionId,
			masterPassword: masterPwd,
			userId,
	...
	@ApiOperation({ summary: 'Delete a panel position from a dashboard' })
	@ApiResponse({
		status: 200,
		description: 'Panel position deleted.',
		type: FoundPanelPositionDto,
	})
	@ApiParam({ name: 'dashboardId', required: true })
	@ApiParam({ name: 'panelPositionId', required: true })
	@ApiParam({ name: 'connectionId', required: true })
	@UseGuards(ConnectionEditGuard)
	@Delete('/dashboard/:dashboardId/panel-position/:panelPositionId/:connectionId')
	async deletePanelPosition(
		@SlugUuid('connectionId') connectionId: string,
		@Param('dashboardId') dashboardId: string,
		@Param('panelPositionId') panelPositionId: string,
		@MasterPassword() masterPwd: string,
	...
	@ApiOperation({
		summary: 'Generate a panel position using AI',
		description:
			'Generates panel configuration with SQL query and chart settings using AI based on natural language description',
	})
	@ApiResponse({
		status: 201,
	...
	@UseGuards(ConnectionEditGuard)
	@Post('/dashboard/:dashboardId/panel-position/generate/:connectionId')
	async generatePanelPositionWithAi(
		@SlugUuid('connectionId') connectionId: string,
		@Param('dashboardId') dashboardId: string,
		@Query('tableName') tableName: string,
		@MasterPassword() masterPwd: string,
