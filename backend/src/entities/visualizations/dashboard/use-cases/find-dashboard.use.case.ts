	...
	implements IFindDashboard
{
	...
	public async implementation(inputData: FindDashboardDs): Promise<FoundDashboardDto> {
		...
		const foundDashboard = await this._dbContext.dashboardRepository.findDashboardWithWidgets(dashboardId);

		if (!foundDashboard || foundDashboard.connection_id !== connectionId) {
			throw new NotFoundException(Messages.DASHBOARD_NOT_FOUND);
		}

		return buildFoundDashboardDto(foundDashboard);
	}
}
