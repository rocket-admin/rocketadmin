	...
	implements IDeleteSavedDbQuery
{
	...
	public async implementation(inputData: FindSavedDbQueryDs): Promise<FoundSavedDbQueryDto> {
		...
		const deletedQueryDto = buildFoundSavedDbQueryDto(foundQuery);
		await this._dbContext.savedDbQueryRepository.remove(foundQuery);
