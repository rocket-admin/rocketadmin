...
export class S3WidgetController {
	...
	async getFileUrl(
		...
		if (!rowPrimaryKeyStr) {
			throw new HttpException({ message: 'Row primary key is required' }, HttpStatus.BAD_REQUEST);
		}
		...
		try {
			...
			throw new HttpException({ message: 'Invalid row primary key format' }, HttpStatus.BAD_REQUEST);
		}
