...
export class TableActionV2Controller {
  ...
  async findTableActions(
    ...
    const inputData: CreateTableActionWithEventAndRuleDS = {
      ...
      table_action_data: {
        action_emails: createTableActionData.table_action_data.emails,
        action_method: createTableActionData.table_action_data.action_method,
        action_type: createTableActionData.table_action_data.action_type,
        action_url: createTableActionData.table_action_data.url,
        action_slack_url: createTableActionData.table_action_data.slack_url,
      },
      action_rules_data: createTableActionData.action_rules_data.map((rule) => ({
        table_name: rule.table_name,
        title: rule.title,
        events_data: rule.events_data.map((event_data) => ({
