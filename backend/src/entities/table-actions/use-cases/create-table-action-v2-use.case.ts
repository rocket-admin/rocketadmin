  ...
  implements ICreateTableActionV2
{
  ...
  protected async implementation(
    ...
    let savedTableAction = await this._dbContext.tableActionRepository.saveNewOrUpdatedTableAction(newTableAction);
    for (const rule of action_rules_data) {
      const { events_data } = rule;
      const newRule = buildTableActionRule(rule);
      let savedRule = await this._dbContext.actionRulesRepository.saveNewOrUpdatedActionRules(newRule);
      ...
      savedRule = await this._dbContext.actionRulesRepository.saveNewOrUpdatedActionRules(newRule);
      savedTableAction.action_rules.push(savedRule);
    }
    savedTableAction = await this._dbContext.tableActionRepository.saveNewOrUpdatedTableAction(newTableAction);

    const savedRules = savedTableAction.action_rules;
