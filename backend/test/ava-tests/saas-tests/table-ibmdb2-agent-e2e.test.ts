...
test.before(async () => {
	...
	connectionToTestDB = getTestData(mockFactory).connectionToAgentIbmDB2;
	...
	// const connectionToIbmDb2 = getTestData(mockFactory).connectionToIbmDb2;
	// const testTableCreationResult = await createTestTable(connectionToIbmDb2);
...
test.serial(
	...
	async (t) => {
		...
		const { message } = updateRowInTableRO;
...
test.serial(`${currentTest} should import csv file with table data`, async (t) => {
	...
	function deleteIdColumnsInCSVFile(filePatch: string) {
		...
		const idColumnIndex = rows[0].split(',').indexOf('ID');
