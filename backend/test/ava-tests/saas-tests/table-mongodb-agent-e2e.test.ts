...
test.serial(`${currentTest} should return rows of selected table without search and without pagination`, async (t) => {
	try {
		...
		t.is(Object.hasOwn(getTableRowsRO.rows[0], '_id'), true);
...
test.serial(`${currentTest} should return rows of selected table with search and without pagination`, async (t) => {
	try {
		...
		const getTableRowsResponse = await request(app.getHttpServer())
			.post(`/table/rows/find/${createdConnectionId}?tableName=${testTableName}&search=${searchedDescription}`)
			.set('Cookie', firstUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');
...
test.serial(`${currentTest} should add row in table and return result`, async (t) => {
	...
	const addedRow = rows.find(
		(r) =>
			r[testTableColumnName] === row[testTableColumnName] &&
			r[testTableSecondColumnName] === row[testTableSecondColumnName],
	);
	t.truthy(addedRow, 'Added row should exist');
	...
	t.is(Object.hasOwn(getLogsRO.logs[addRowLogIndex].affected_primary_key, '_id'), true);
});
...
test.serial(`${currentTest} should update row in table and return result`, async (t) => {
	...
	const updateRowInTableResponse = await request(app.getHttpServer())
		.put(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=${foundIdForUpdate}`)
		.send(JSON.stringify(row))
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception when connection id not passed in request`, async (t) => {
	...
	const emptyConnectionId = '';
	const updateRowInTableResponse = await request(app.getHttpServer())
		.put(`/table/row/${emptyConnectionId}?tableName=${testTableName}&_id=1`)
		.send(JSON.stringify(row))
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception when connection id passed in request is incorrect`, async (t) => {
	...
	const fakeConnectionId = faker.string.uuid();
	const updateRowInTableResponse = await request(app.getHttpServer())
		.put(`/table/row/${fakeConnectionId}?tableName=${testTableName}&_id=1`)
		.send(JSON.stringify(row))
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception when tableName not passed in request`, async (t) => {
	...
	const fakeConnectionId = faker.string.uuid();
	const updateRowInTableResponse = await request(app.getHttpServer())
		.put(`/table/row/${fakeConnectionId}?tableName=&_id=1`)
		.send(JSON.stringify(row))
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception when tableName passed in request is incorrect`, async (t) => {
	...
	const updateRowInTableResponse = await request(app.getHttpServer())
		.put(`/table/row/${createdConnectionId}?tableName=${fakeTableName}&_id=1`)
		.send(JSON.stringify(row))
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(
	...
	async (t) => {
		...
		const updateRowInTableResponse = await request(app.getHttpServer())
			.put(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=6606c97e2fff2ec599cc7bca`)
			.send(JSON.stringify(row))
			.set('Cookie', firstUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');
...
test.serial(`${currentTest} should update multiple rows and return result`, async (t) => {
	...
	const firstRowResponse = await request(app.getHttpServer())
		.get(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=${foundFirstIdForUpdate}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
	...
	const secondRowResponse = await request(app.getHttpServer())
		.get(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=${foundSecondIdForUpdate}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should delete row in table and return result`, async (t) => {
	...
	const deleteRowInTableResponse = await request(app.getHttpServer())
		.delete(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=${idForDeletion}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception when tableName not passed in request`, async (t) => {
	...
	const fakeTableName = '';
	const deleteRowInTableResponse = await request(app.getHttpServer())
		.delete(`/table/row/${createdConnectionId}?tableName=${fakeTableName}&_id=${idForDeletion}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(
	...
	async (t) => {
		const deleteRowInTableResponse = await request(app.getHttpServer())
			.delete(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=6606c97e2fff2ec599cc7bca`)
			.set('Cookie', firstUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');
...
test.serial(`${currentTest} found row`, async (t) => {
	...
	const foundRowInTableResponse = await request(app.getHttpServer())
		.get(`/table/row/${createdConnectionId}?tableName=${testTableName}&_id=${idForSearch}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception, when connection id is not passed in request`, async (t) => {
	...
	const emptyConnectionId = '';
	const foundRowInTableResponse = await request(app.getHttpServer())
		.get(`/table/row/${emptyConnectionId}?tableName=${testTableName}&_id=${idForSearch}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(
	...
	async (t) => {
		...
		const fakeConnectionId = faker.string.uuid();
		const foundRowInTableResponse = await request(app.getHttpServer())
			.get(`/table/row/${fakeConnectionId}?tableName=${testTableName}&_id=${idForSearch}`)
			.set('Cookie', firstUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');
...
test.serial(`${currentTest} should throw an exception, when tableName in not passed in request`, async (t) => {
	...
	const fakeTableName = '';
	const foundRowInTableResponse = await request(app.getHttpServer())
		.get(`/table/row/${createdConnectionId}?tableName=${fakeTableName}&_id=${idForSearch}`)
		.set('Cookie', firstUserToken)
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} should import csv file with table data`, async (t) => {
	...
	function deleteIdColumnsInCSVFile(filePatch: string) {
		...
		const idColumnIndex = rows[0].split(',').indexOf('_id');
