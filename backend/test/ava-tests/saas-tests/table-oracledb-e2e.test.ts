...
should return all found rows with search, pagination: page=1, perPage=2 and DESC sorting and filtering`,
	async (t) => {
		try {
			...
			const _createTableSettingsDTO = mockFactory.generateTableSettings(
				createConnectionRO.id,
				testTableName,
				[testTableColumnName],
				undefined,
			...
			const _secondFieldName = 'updated_at';
...
test.serial(`${currentTest} should add row in table with with different data`, async (t) => {
	...
	const _fakeName = faker.person.firstName();
	const _fakeMail = faker.internet.email();
