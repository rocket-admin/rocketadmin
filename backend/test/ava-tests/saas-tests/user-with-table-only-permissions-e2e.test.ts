...
test.serial(`${currentTest} should throw ${Messages.DONT_HAVE_PERMISSIONS} exception object when you update permissions`, async (t) => {
  try {
    const testData = await createConnectionsAndInviteNewUserInNewGroupWithOnlyTablePermissions(app);
    const {
      connections,
...
test.serial(`${currentTest} should exception ${Messages.DONT_HAVE_PERMISSIONS} when table settings was not created`, async (t) => {
  try {
    const testData = await createConnectionsAndInviteNewUserInNewGroupWithOnlyTablePermissions(app);
    const {
      connections,
