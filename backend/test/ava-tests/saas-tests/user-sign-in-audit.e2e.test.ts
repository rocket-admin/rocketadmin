...
test.serial(`${currentTest} should not return logs from another company`, async (t) => {
  try {
    // Register first user
    const firstUserInfo = await registerUserAndReturnUserInfo(app);
    const { token: firstToken, email: firstEmail, password: firstPassword } = firstUserInfo;

    // Get first user's company ID
    const firstCompanyId = await getCompanyIdByToken(app, firstToken);

    // Register second user (different company)
    const secondUserInfo = await registerUserAndReturnUserInfo(app);
    const { token: secondToken, email: secondEmail, password: secondPassword } = secondUserInfo;

    // Get second user's company ID
    const secondCompanyId = await getCompanyIdByToken(app, secondToken);

    // Create login for first user
    await request(app.getHttpServer())
      .post('/user/login/')
    ...
    // Get sign-in audit logs for first company
    const getFirstCompanyLogsResult = await request(app.getHttpServer())
      .get(`/sign-in-audit/logs?companyId=${firstCompanyId}`)
      .set('Cookie', firstToken)
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');

    t.is(getFirstCompanyLogsResult.status, 200);
    const firstCompanyLogsRO = JSON.parse(getFirstCompanyLogsResult.text);

    // First company logs should not contain second user's email
    const secondUserLogInFirstCompany = firstCompanyLogsRO.logs.find(
      (log: any) => log.email === secondEmail.toLowerCase(),
    );
    t.falsy(secondUserLogInFirstCompany);

    // Get sign-in audit logs for second company
    const getSecondCompanyLogsResult = await request(app.getHttpServer())
      .get(`/sign-in-audit/logs?companyId=${secondCompanyId}`)
      .set('Cookie', secondToken)
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json');

    t.is(getSecondCompanyLogsResult.status, 200);
    const secondCompanyLogsRO = JSON.parse(getSecondCompanyLogsResult.text);

    // Second company logs should not contain first user's email
    const firstUserLogInSecondCompany = secondCompanyLogsRO.logs.find(
      (log: any) => log.email === firstEmail.toLowerCase(),
    );
    t.falsy(firstUserLogInSecondCompany);
  } catch (err) {
    console.error(err);
    throw err;
  }
