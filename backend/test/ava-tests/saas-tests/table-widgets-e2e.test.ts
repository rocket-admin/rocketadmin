...
test.serial(
	`${currentTest} should return empty array, table widgets not created`,
	async (t) => {
...
test.skip(`${currentTest} should return automatically created widgets array, if table contains specific data`, async (t) => {
	const connectionToTestDB = getTestData(mockFactory).connectionToPostgres;
	const firstUserToken = (await registerUserAndReturnUserInfo(app)).token;
	const { testTableName } = await createTestTable(
		connectionToTestDB,
		undefined,
		undefined,
		false,
	...
	const phoneWidget = getTableWidgetsRO.find(
		(w) => w.widget_type === WidgetTypeEnum.Phone,
	);
	t.truthy(phoneWidget);
	const uuidWidget = getTableWidgetsRO.find(
		(w) => w.widget_type === WidgetTypeEnum.UUID,
	);
	t.truthy(uuidWidget);
	const countryCodeWidget = getTableWidgetsRO.find(
		(w) => w.widget_type === WidgetTypeEnum.Country,
	);
	t.truthy(countryCodeWidget);
	const emailWidget = getTableWidgetsRO.find(
		(w) => w.widget_type === WidgetTypeEnum.String,
	);
	t.truthy(emailWidget);
	t.is(emailWidget.widget_params.includes('"validate": "isEmail"'), true);
	const urlWidget = getTableWidgetsRO.find(
		(w) => w.widget_type === WidgetTypeEnum.URL,
	);
	t.truthy(urlWidget);
	const foundColorWIdgets = getTableWidgetsRO.filter(
		(w) => w.widget_type === WidgetTypeEnum.Color,
	);
...
test.serial(
	...
	async (t) => {
		...
		const newTableWidgets =
			mockFactory.generateCreateWidgetDTOsArrayForConnectionTable();
...
test.serial(
	`${currentTest} should throw exception when connection id not passed in request`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(
	`${currentTest} should throw exception when connection id passed in request is incorrect`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(
	`${currentTest} should throw exception when tableName passed in request is incorrect`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(
	`${currentTest} should throw exception when tableName not passed in request`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(`${currentTest} should return created table widgets`, async (t) => {
	...
	const newTableWidget =
		mockFactory.generateCreateWidgetDTOForConnectionTable();
	const createTableWidgetResponse = await request(app.getHttpServer())
...
test.serial(
	`${currentTest} should return updated table widgets when old widget updated and new added`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(
	`${currentTest} should return table widgets without deleted widget`,
	async (t) => {
		const { token } = await registerUserAndReturnUserInfo(app);
		const newConnection = getTestData(mockFactory).newEncryptedConnection;
		const createdConnection = await request(app.getHttpServer())
...
test.serial(
	...
	async (t) => {
		...
		t.is(createTableWidgetResponse.status, 400);
		t.is(
			createTableWidgetRO.message,
			Messages.EXCLUDED_OR_NOT_EXISTS(copyWidgets[0].field_name),
		);
	},
);
...
test.serial(
	...
	async (t) => {
		...
		t.is(
			getTableWidgetsRO[0].widget_type,
			foreignKeyWidgetsDTO.widgets[0].widget_type,
		);

		const getTableStructureResponse = await request(app.getHttpServer())
			.get(
				`/table/structure/${connectionId}?tableName=${referencedTableTableName}`,
			)
			.set("Content-Type", "application/json")
			.set("Cookie", token)
			.set("Accept", "application/json");
		...
		t.is(
			getTableStructureRO.table_widgets[0].field_name,
			foreignKeyWidgetsDTO.widgets[0].field_name,
		);
		t.is(
			getTableStructureRO.table_widgets[0].widget_type,
			foreignKeyWidgetsDTO.widgets[0].widget_type,
		);
		t.is(Object.hasOwn(getTableStructureRO, "foreignKeys"), true);
		t.is(getTableStructureRO.foreignKeys.length, 1);
		t.is(
			getTableStructureRO.foreignKeys[0].column_name,
			foreignKeyWidgetsDTO.widgets[0].field_name,
		);
		t.is(
			getTableStructureRO.foreignKeys[0].referenced_table_name,
			firstTableData.testTableName,
		);
		const widgetParams = JSON5.parse(
		...
		t.is(
			Object.hasOwn(getTableStructureRO.foreignKeys[0], "autocomplete_columns"),
			true,
		);
...
test.serial(
	...
	async (t) => {
		...
		if (tableExists.length && tableExists[0]["1"]) {
			await ibmDatabase.query(
				`DROP TABLE ${connectionToTestDB.schema}.${referencedByTableTableName}`,
			);
		}
    ...
    CREATE TABLE ${connectionToTestDB.schema}.${referencedByTableTableName} (
      ...
      PRIMARY KEY (id)
		...
		const getTableStructureResponse = await request(app.getHttpServer())
			.get(
				`/table/structure/${connectionId}?tableName=${referencedByTableTableName}`,
			)
			.set("Content-Type", "application/json")
			.set("Cookie", token)
			.set("Accept", "application/json");
		...
		const getRowsResponse = await request(app.getHttpServer())
			.get(
				`/table/rows/${connectionId}?tableName=${referencedByTableTableName}`,
			)
			.set("Content-Type", "application/json")
			.set("Cookie", token)
			.set("Accept", "application/json");
...
test.serial(
	...
	async (t) => {
		...
		const getTableStructureResponse = await request(app.getHttpServer())
			.get(
				`/table/structure/${connectionId}?tableName=${referencedByTableName}`,
			)
			.set("Content-Type", "application/json")
			.set("Cookie", token)
			.set("Accept", "application/json");
