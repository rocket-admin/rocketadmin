...
currentTest = 'POST /dashboard/:dashboardId/panel-position/generate/:connectionId';

test.serial(`${currentTest} should generate a panel position with AI for chart type`, async (t) => {
	...
	const generateWidget = await request(app.getHttpServer())
		.post(`/dashboard/${dashboardId}/panel-position/generate/${connectionId}?tableName=${testTableName}`)
		.send({
...
test.serial(`${currentTest} should generate a counter panel position with AI`, async (t) => {
...
test.serial(`${currentTest} should generate panel position with custom name`, async (t) => {
	mockResponse = MOCK_AI_RESPONSE_CHART;
...
test.serial(`${currentTest} should fail for non-existent dashboard`, async (t) => {
	...
	const generateWidget = await request(app.getHttpServer())
		.post(`/dashboard/${fakeDashboardId}/panel-position/generate/${connectionId}?tableName=${testTableName}`)
		.send({
			chart_description: 'Show some data',
		})
		.set('Cookie', token)
...
test.serial(`${currentTest} should fail for non-existent table`, async (t) => {
	...
	const generateWidget = await request(app.getHttpServer())
		.post(`/dashboard/${dashboardId}/panel-position/generate/${connectionId}?tableName=non_existent_table_xyz`)
		.send({
			chart_description: 'Show some data',
		})
		.set('Cookie', token)
