...
test.serial(`${currentTest} should add row in table and return result`, async (t) => {
	...
	console.log('🚀 ~ test.serial ~ getLogsRO:', getLogsRO.logs[1].affected_primary_key);
...
test.serial(`${currentTest} should update row in table and return result, when table has json field`, async (t) => {
	...
	t.is(Object.hasOwn(updateNewRowInTableRO, 'row'), true);
	t.is(Object.hasOwn(updateNewRowInTableRO, 'structure'), true);
	t.is(Object.hasOwn(updateNewRowInTableRO, 'foreignKeys'), true);
	t.is(Object.hasOwn(updateNewRowInTableRO, 'primaryColumns'), true);
	t.is(Object.hasOwn(updateNewRowInTableRO, 'readonly_fields'), true);
