...
test.serial(`${currentTest} should enforce saved cedar policy - user with table:add can add rows`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
		const connectionId = testData.connections.firstId;
		const groupId = testData.groups.createdGroupId;
...
test.serial(`${currentTest} should reject request when user does not have connection edit access`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
		const connectionId = testData.connections.firstId;
		const groupId = testData.groups.createdGroupId;
...
test.serial(`${currentTest} should reject empty cedar policy string`, async (t) => {
	try {
		...
		const response = await request(app.getHttpServer())
			.post(`/connection/cedar-policy/${connectionId}`)
			.send({ cedarPolicy: '', groupId: testData.groups.firstAdminGroupId })
			.set('Cookie', testData.users.adminUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');
...
test.serial(`${currentTest} should reject saving cedar policy on admin (isMain) group`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInAdminGroupOfFirstConnection(app);
		const connectionId = testData.connections.firstId;
		...
		const response = await request(app.getHttpServer())
			.post(`/connection/cedar-policy/${connectionId}`)
			.send({ cedarPolicy, groupId: adminGroupId })
			.set('Cookie', testData.users.adminUserToken)
			.set('Content-Type', 'application/json')
			.set('Accept', 'application/json');

		t.is(response.status, 400);
		t.is(JSON.parse(response.text).message, Messages.CANNOT_CHANGE_ADMIN_GROUP);
	} catch (error) {
		console.error(error);
		throw error;
	}
...
test.serial(`${currentTest} should reject saving cedar policy when group does not belong to connection`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
...
test.serial(`${currentTest} should overwrite previous permissions when saving new cedar policy`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
		const connectionId = testData.connections.firstId;
		const groupId = testData.groups.createdGroupId;
...
test.serial(`${currentTest} should reject cedar policy that references a foreign connection`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
		const connectionId = testData.connections.firstId;
		const secondConnectionId = testData.connections.secondId;
...
test.serial(`${currentTest} should reject cedar policy that references a table from another connection`, async (t) => {
	try {
		const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
		const connectionId = testData.connections.firstId;
		const secondConnectionId = testData.connections.secondId;
...
test.serial(
	`${currentTest} should reject cedar policy that references a dashboard from another connection`,
	async (t) => {
		try {
			const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
			const connectionId = testData.connections.firstId;
...
test.serial(
	`${currentTest} should allow cedar policy that references a valid group from the same connection as resource`,
	async (t) => {
		try {
			const testData = await createConnectionsAndInviteNewUserInNewGroupWithGroupPermissions(app);
			const connectionId = testData.connections.firstId;
			...
			t.is(response.status, 201);
			t.truthy(response.body.cedarPolicy);
			t.truthy(response.body.classicalPermissions);
		} catch (error) {
			console.error(error);
			throw error;
		}
