...
async function setupS3WidgetTestEnvironment(
	options: { createSecrets?: boolean; createWidget?: boolean; s3FieldName?: string } = {},
): Promise<{
	token: string;
	connectionId: string;
	tableName: string;
	...
	const { createSecrets = true, createWidget = true, s3FieldName = 'file_key' } = options;
...
test.serial(`${currentTest} - should return 403 when user tries to access another user's connection`, async (t) => {
	// First user creates connection
	const { connectionId, tableName, fieldName, rowPrimaryKey } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 403 when using fake connection ID`, async (t) => {
	const { token } = await registerUserAndReturnUserInfo(app);
...
test.serial(`${currentTest} - should return 400 when tableName is missing`, async (t) => {
	const { token, connectionId, fieldName, rowPrimaryKey } = await setupS3WidgetTestEnvironment();

	const response = await request(app.getHttpServer())
		.get(`/s3/file/${connectionId}?fieldName=${fieldName}&rowPrimaryKey=${JSON.stringify(rowPrimaryKey)}`)
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} - should return 400 when fieldName is missing`, async (t) => {
	const { token, connectionId, tableName, rowPrimaryKey } = await setupS3WidgetTestEnvironment();

	const response = await request(app.getHttpServer())
		.get(`/s3/file/${connectionId}?tableName=${tableName}&rowPrimaryKey=${JSON.stringify(rowPrimaryKey)}`)
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} - should return 400 when rowPrimaryKey is missing`, async (t) => {
	const { token, connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();

	const response = await request(app.getHttpServer())
		.get(`/s3/file/${connectionId}?tableName=${tableName}&fieldName=${fieldName}`)
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} - should return 400 when rowPrimaryKey has invalid JSON format`, async (t) => {
	const { token, connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();

	const response = await request(app.getHttpServer())
		.get(`/s3/file/${connectionId}?tableName=${tableName}&fieldName=${fieldName}&rowPrimaryKey=invalid-json`)
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
		.set('Accept', 'application/json');
...
test.serial(`${currentTest} - should return 400 when S3 widget is not configured for the field`, async (t) => {
	const { token, connectionId, tableName, rowPrimaryKey } = await setupS3WidgetTestEnvironment({
		createWidget: false,
	});
...
test.serial(`${currentTest} - should return 404 when row with primary key not found`, async (t) => {
	const { token, connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 404 when AWS credentials secrets are not found`, async (t) => {
	const { token, connectionId, tableName, fieldName, rowPrimaryKey } = await setupS3WidgetTestEnvironment({
		createSecrets: false,
	});
...
test.serial(`${currentTest} - should return 403 when user tries to access another user's connection`, async (t) => {
	// First user creates connection
	const { connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();

	// Second user tries to access first user's connection
	const { token: secondUserToken } = await registerUserAndReturnUserInfo(app);

	const response = await request(app.getHttpServer())
		.post(`/s3/upload-url/${connectionId}?tableName=${tableName}&fieldName=${fieldName}`)
...
test.serial(`${currentTest} - should return 403 when using fake connection ID`, async (t) => {
	...
	const response = await request(app.getHttpServer())
		.post(`/s3/upload-url/${fakeConnectionId}?tableName=test_table&fieldName=file_key`)
		.send({ filename: 'test.pdf', contentType: 'application/pdf' })
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
...
test.serial(`${currentTest} - should return 400 when tableName is missing`, async (t) => {
	const { token, connectionId, fieldName } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 400 when fieldName is missing`, async (t) => {
	const { token, connectionId, tableName } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 400 when filename is missing`, async (t) => {
	const { token, connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 400 when contentType is missing`, async (t) => {
	const { token, connectionId, tableName, fieldName } = await setupS3WidgetTestEnvironment();
...
test.serial(`${currentTest} - should return 400 when S3 widget is not configured for the field`, async (t) => {
	...
	const response = await request(app.getHttpServer())
		.post(`/s3/upload-url/${connectionId}?tableName=${tableName}&fieldName=file_key`)
		.send({ filename: 'test.pdf', contentType: 'application/pdf' })
		.set('Cookie', token)
		.set('masterpwd', 'ahalaimahalai')
		.set('Content-Type', 'application/json')
...
test.serial(`${currentTest} - user A cannot get file URL from user B's connection`, async (t) => {
...
test.serial(`${currentTest} - user A cannot get upload URL from user B's connection`, async (t) => {
	// User A sets up their environment
	const userA = await setupS3WidgetTestEnvironment();
	...
	const response = await request(app.getHttpServer())
		.post(`/s3/upload-url/${userA.connectionId}?tableName=${userA.tableName}&fieldName=${userA.fieldName}`)
...
test.serial(`${currentTest} - user can access their own connection successfully`, async (t) => {
...
test.serial(`${currentTest} - unauthenticated user cannot access S3 endpoints`, async (t) => {
	const { connectionId, tableName, fieldName, rowPrimaryKey } = await setupS3WidgetTestEnvironment();
