...
//****************************** CEDAR MIGRATION: existing users retain permissions after migration ******************************

currentTest = 'Cedar migration - admin user';
...
test.serial(
	...
	async (t) => {
		try {
			...
			// Step 2: Verify classical permissions work (admin can read tables)
...
currentTest = 'Cedar migration - non-admin user with readonly permissions';

test.serial(
	`${currentTest} should retain readonly access for non-admin user after Cedar migration`,
	async (t) => {
		try {
			// Step 1: Set up WITHOUT Cedar - admin creates connection, group, invites user with readonly permissions
			const adminUserInfo = await registerUserAndReturnUserInfo(app);
			const adminUserToken = adminUserInfo.token;
...
currentTest = 'Cedar migration - non-admin user with edit permissions';
...
currentTest = 'Cedar migration - user with connection edit permissions';
...
test.serial(
	...
	async (t) => {
		try {
			...
			// Create a non-admin group with connection edit permissions
			const newGroup = mockFactory.generateCreateGroupDto1();
			const createGroupResponse = await request(app.getHttpServer())
				.post(`/connection/group/${connectionId}`)
				.set('Cookie', adminUserToken)
			...
			// Set connection edit permissions
			const permissions = {
				connection: {
					connectionId: connectionId,
			...
			// Step 2: Verify classical permissions work
			const getConnectionBeforeMigration = await request(app.getHttpServer())
				.get(`/connection/one/${connectionId}`)
				.set('Cookie', simpleUserToken)
				.set('Content-Type', 'application/json')
				.set('Accept', 'application/json');
			t.is(getConnectionBeforeMigration.status, 200);
			...
			// Step 5: Verify user retains connection read access via Cedar
			const getConnectionAfterMigration = await request(app.getHttpServer())
				.get(`/connection/one/${connectionId}`)
				.set('Cookie', simpleUserToken)
				.set('Content-Type', 'application/json')
				.set('Accept', 'application/json');
			t.is(getConnectionAfterMigration.status, 200);
...
currentTest = 'Cedar migration - user added to admin group of existing connection';
...
currentTest = 'Cedar migration - multiple users with different permission levels';
...
currentTest = 'Cedar migration - GET /connections';
...
currentTest = 'Cedar migration - GET /connection/groups/:slug';
...
currentTest = 'Cedar migration - GET /connection/one/:slug';
...
currentTest = 'Cedar migration - demo connections access';
