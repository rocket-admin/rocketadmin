...
const _newConnectionToPostgres =
...
test.serial(
	...
	async (t) => {
		try {
			const testData =
				await createConnectionsAndInviteNewUserInNewGroupWithOnlyTablePermissions(
					app,
				);
			const {
				connections,
			...
			const _resultRO = JSON.parse(findAll.text);
...
test.serial(
	`${currentTest} it should throw exception ${Messages.DONT_HAVE_PERMISSIONS}`,
	async (t) => {
		try {
			const testData =
				await createConnectionsAndInviteNewUserInNewGroupWithOnlyTablePermissions(
			...
			t.is(Object.hasOwn(getUsersRO, "message"), true);
...
test.serial(
	...
	async (t) => {
		try {
			...
			const _connectionPropertiesResult = JSON.parse(
...
test.serial(
	`${currentTest} 'should should throw exception ${Messages.DONT_HAVE_PERMISSIONS}`,
	async (t) => {
		try {
			const testData =
				await createConnectionsAndInviteNewUserInNewGroupWithOnlyTablePermissions(
