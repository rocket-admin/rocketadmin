...
xdescribe('Tables Postgres Internal Database (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    newConnection = mockFactory.generateCreateInternalConnectionDto();
    newConnection2 = mockFactory.generateCreateConnectionDto();
  ...
  afterAll(async () => {
    try {
      ...
      console.error('After all table-postgres-internal error: ' + e);
    }
  });
    ...
    describe('without search and without pagination and without sorting', () => {
      it('should return rows of selected table without search and without pagination', async () => {
        try {
          ...
          expect(getTableRowsRO.rows[3].hasOwnProperty('schema')).toBeTruthy();
    ...
    describe('with search, without pagination and without sorting', () => {
      it('should return rows of selected table with search and without pagination', async () => {
        try {
          ...
          expect(decryptValue(getTableRowsRO.rows[0].username)).toBe(newConnection2.username);
          expect(decryptValue(getTableRowsRO.rows[0].password)).toBe(newConnection2.password);
          expect(decryptValue(getTableRowsRO.rows[0].database)).toBe(newConnection2.database);
    ...
    describe('without search, with pagination, without sorting', () => {
      it('should return page of all rows with pagination page=1, perPage=2', async () => {
        try {
          ...
          expect(getTableRowsRO.rows[1].hasOwnProperty('schema')).toBeTruthy();
    ...
    describe('with search, with pagination, without sorting', () => {
      it('should return all found rows with pagination page=1 perPage=2', async () => {
        try {
          ...
          expect(decryptValue(getTableRowsRO.rows[1].username)).toBe(newConnection2.username);
          expect(decryptValue(getTableRowsRO.rows[0].password)).toBe(newConnection2.password);
          expect(decryptValue(getTableRowsRO.rows[1].database)).toBe(newConnection2.database);
          expect(getTableRowsRO.rows[0].sid).toBe(null);
